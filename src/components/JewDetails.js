...
function JewDetails({ selectedGems, minedGems, jewelry, account, jewelryContract, gemstoneSelectingContract, gemstoneExtractionContract }) {
  ...
  const handlePrevMinedGem = () => {
    setCurrentMinedGemIndex(prev => Math.max(prev - 1, 0));
  };

  const handleNextMinedGem = () => {
    setCurrentMinedGemIndex(prev => Math.min(prev + 1, minedGems.length - 1));
  };

  const handlePrevSelectedGem = () => {
    setCurrentSelectedGemIndex(prev => Math.max(prev - 1, 0));
  };

  const handleNextSelectedGem = () => {
    setCurrentSelectedGemIndex(prev => Math.min(prev + 1, selectedGems.length - 1));
  };
  ...
  return (
    <div className="details-details-container">
      ...
      <div className="details-row">
        <span className="arrow" onClick={handlePrevMinedGem}>
          ←
        </span>
        <div className="card-container">
          {renderMinedGems()[currentMinedGemIndex]}
        </div>
        <span className="arrow" onClick={handleNextMinedGem}>
          →
        </span>
      </div>

      <div className="details-row">
        <span className="arrow" onClick={handlePrevSelectedGem}>
          ←
        </span>
        <div className="card-container">
          {renderSelectedGems()[currentSelectedGemIndex]}
        </div>
        <span className="arrow" onClick={handleNextSelectedGem}>
          →
        </span>
      </div>
...
function JewDetails({ selectedGems, minedGems, jewelry, account, jewelryContract, gemstoneSelectingContract, gemstoneExtractionContract }) {
  ...
  const [currentGemIndex, setCurrentGemIndex] = useState(0); // Single index for both mined and selected gems
  ...
  const renderMinedGems = () => {
    const filteredMinedGems = minedGems.filter(gem => prevGemsArray.includes(parseInt(gem.id, 10))).reverse(); // Reverse mined gems
    if (filteredMinedGems.length === 0) {
      return <p>No Mined Gems Available</p>;
    }
  ...
  const renderSelectedGems = () => {
    const filteredSelectedGems = selectedGems.filter(gem => prevGemsArray.includes(parseInt(gem.id, 10))).reverse(); // Reverse selected gems
    if (filteredSelectedGems.length === 0) {
      return <p>No Selected Gems Available</p>;
    }
  ...
  const handlePrevGem = () => {
    const maxIndex = Math.min(minedGems.length, selectedGems.length) - 1;
    setCurrentGemIndex(prev => (prev === 0 ? maxIndex : prev - 1)); // Go to last gem if at the beginning
  };

  const handleNextGem = () => {
    const maxIndex = Math.min(minedGems.length, selectedGems.length) - 1;
    setCurrentGemIndex(prev => (prev === maxIndex ? 0 : prev + 1)); // Go to first gem if at the end
  };
